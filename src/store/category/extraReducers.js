...
export const categoryExtraReducers = (builder) => {
	...
	builder
		.addCase(enableCategoryById.pending, (state) => {
			state.statusActive = 'Cargando';
		})
		.addCase(enableCategoryById.fulfilled, (state, action) => {
			state.statusActive = 'Exitoso';
			state.categories = state.categories.map((category) =>
				category.uid === action.payload.uid ? action.payload : category
			);
		})
		.addCase(enableCategoryById.rejected, (state, action) => {
			state.statusActive = 'Fallido';
			state.error = action.payload;
		});

	builder
		.addCase(disableCategoryById.pending, (state) => {
			state.statusActive = 'Cargando';
		})
		.addCase(disableCategoryById.fulfilled, (state, action) => {
			state.statusActive = 'Exitoso';
			state.categories = state.categories.map((category) =>
				category.uid === action.payload.uid ? action.payload : category
			);
		})
		.addCase(disableCategoryById.rejected, (state, action) => {
			state.statusActive = 'Fallido';
			state.error = action.payload;
		});
