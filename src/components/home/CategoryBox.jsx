    ...
    <>
      {showFull ? (
        <Link
          ...
          className='flex flex-col lg:flex-row py-2 h-36 lg:h-28 lg:items-center px-4 justify-between bg-[#2e2e2e] border-l-4 mx-2'
        ...
        <Link
          to={`/topic-list?category=${category.uid}`}
          className='flex flex-col h-28 lg:h-24 items-start px-4 justify-center bg-[#2e2e2e] border-l-4'
          style={{ borderLeft: `10px solid ${category.color}` }}
        >
          <div className='flex flex-col'>
            <p className='lg:text-lg font-semibold text-white'>{category.title}</p>
            <p className='text-sm text-gray-300 leading-4 lg:leading-normal'>
              {category.description}
            </p>
          </div>
